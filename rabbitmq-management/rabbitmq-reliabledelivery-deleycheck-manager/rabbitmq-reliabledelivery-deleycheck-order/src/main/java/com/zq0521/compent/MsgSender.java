...
public class MsgSender implements InitializingBean {
     ...
     * 发送延时消息
     */
    public void sendDelayCheckMsg(MsgTxtBo msgTxtBo) {
        log.info("发送延时消息ID：{}", msgTxtBo.getMsgId());
        //表示为延时消息
        CorrelationData correlationData = new CorrelationData(msgTxtBo.getMsgId()+"_"+msgTxtBo.getOrderNo()+"_delay");
        rabbitTemplate.convertAndSend(MqConstants.ORDER_TO_PRODUCT_DELAY_EXCHANGE_NAME, MqConstants.ORDER_TO_PRODUCT_DELAY_ROUTING_KEY, msgTxtBo, new MessagePostProcessor() {
            @Override
            public Message postProcessMessage(Message message) throws AmqpException {
                message.getMessageProperties().setHeader("x-delay", MqConstants.DELAY_TIME);  //设置延迟时间
                return message;
            }
        }, correlationData);
    }
